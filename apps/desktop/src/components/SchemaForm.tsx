...
import type {RecoveryEdit} from '../recovery.ts';

interface FormProps {
  schema: Schema;
  value: Record<string, Json>;
  onChange: (value: Record<string, Json>, edit: RecoveryEdit) => void;
  errors: Record<string, string>;
  // Distinguishes field IDs when two editors of the same schema are on one page; the default is the ordinary editor.
  idPrefix?: string;
  // Value paths whose string is loaded data under a number/integer schema: shown as a type mismatch to replace, not as
  // numeric text to parse. The ordinary editor has none; every string there is the operator's own text.
...
interface FieldProps {
  ...
  value: Json;
  path: string;
  onChange: (value: Json, edit: RecoveryEdit) => void;
  errors: Record<string, string>;
  idPrefix: string;
  stored: ReadonlySet<string> | undefined;
}

function Field({schema, value, path, onChange, errors, idPrefix, stored}: FieldProps) {
    ...
    return <div id={id} className="object-fields" role="group" aria-label={path}>
      {Object.entries(schema.properties ?? {}).map(([name, child]) => {
        ...
        return <section className="schema-field" key={name}>
          ...
          {present ? <Field schema={child} value={value[name]} path={childPath} errors={errors} idPrefix={idPrefix} stored={stored}
            onChange={(next, edit) => onChange({...value, [name]: next}, edit)}/>
      ...
      {Object.keys(value).filter(name => !Object.hasOwn(schema.properties ?? {}, name)).map(name => <div className="inline-warning" key={name}>
        ...
        <button type="button" onClick={() => {const next = {...value}; delete next[name]; onChange(next, {kind:'replace', path:optionPath(path, name)});}}>{t.schema.removeField}</button>
      </div>)}
    </div>;
  }
  if (schema.type === 'array' && Array.isArray(value) && schema.items) {
    function move(index: number, offset: number) {
      ...
      onChange(next, {kind:'move', path, index, other:index + offset});
    }
    return <div id={id} className="array-field" role="group" aria-label={path}>
      ...
      {value.map((item, index) => <div className="array-item" key={index}>
        ...
        <div className="array-value"><Field schema={schema.items!} value={item} path={`${path}[${index}]`} errors={errors} idPrefix={idPrefix} stored={stored}
          onChange={(next, edit) => onChange(value.map((old, at) => at === index ? next : old), edit)}/></div>
        <div className="array-actions">
          ...
          <button type="button" aria-label={t.schema.removeItem(`${path}[${index}]`)} onClick={() => onChange(value.filter((_, at) => at !== index), {kind:'remove', path, index})}>{t.schema.remove}</button>
        </div>
      </div>)}
      <button type="button" disabled={schema.maxItems !== undefined && value.length >= schema.maxItems}
        onClick={() => onChange([...value, emptyValue(schema.items!)], {kind:'replace', path:`${path}[${value.length}]`})}>{t.schema.addItem}</button>
    </div>;
  }
  if (schema.enum && (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean')) {
    ...
    return <Select id={id} aria-label={path} value={index < 0 ? '' : String(index)}
      onChange={selected => {if (selected !== '') onChange(schema.enum![Number(selected)], {kind:'replace', path});}}
  ...
  if (schema.type === 'boolean' && typeof value === 'boolean') {
    return <label className="checkbox-label"><input id={id} aria-label={path} type="checkbox" checked={value}
      onChange={event => onChange(event.target.checked, {kind:'replace', path})}/>{value ? t.schema.enabled : t.schema.disabled}</label>;
  }
  if (schema.type === 'string' && typeof value === 'string') {
    return <div><input id={id} aria-label={path} type="text" value={value} onChange={event => onChange(event.target.value, {kind:'replace', path})}/>
  ...
  if ((schema.type === 'integer' || schema.type === 'number') && (typeof value === 'number' || (typeof value === 'string' && !stored?.has(path)))) {
    return <div><input id={id} aria-label={path} type="text" inputMode="decimal" value={String(value)}
      ...
      onChange={event => onChange(event.target.value, {kind:'replace', path})}/>
  ...
  return <div id={id} className="inline-warning">{t.schema.mismatch(schema.type, JSON.stringify(value))}
    <button type="button" onClick={() => onChange(emptyValue(schema), {kind:'replace', path})}>{t.schema.replace(schema.type)}</button>
    {errors[path] && <p className="field-error">{errors[path]}</p>}
  </div>;
}

export default function SchemaForm({schema, value, onChange, errors, idPrefix = 'option', stored}: FormProps) {
  return <Field schema={schema} value={value} path="$" errors={errors} idPrefix={idPrefix} stored={stored} onChange={(next, edit) => onChange(next as Record<string, Json>, edit)}/>;
}
