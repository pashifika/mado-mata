import type {Fault, Json, ProfileRecoveryOutcome, RecoveryMutation, RecoveryRef, RecoveryView} from './types.ts';
...
function fresh(view:RecoveryView, draftRevision:number):RecoveryState {
  ...
  return {view, selectedId:first?.profile.id ?? null, draft:first ? structuredClone(first.profile.values) : {},
...
export function selectRecovery(state:RecoveryState, id:string|null):RecoveryState {
  ...
  return {...state, selectedId:entry?.profile.id ?? null, draft:entry ? structuredClone(entry.profile.values) : {},
...
export function editRecovery(state:RecoveryState, draft:Record<string,Json>):RecoveryState {
  return {...state, draft, draftRevision:state.draftRevision + 1, touched:true, issue:null};
}
...
// Only a current reply may replace the draft; context retirement requires explicit Close/Reinspect.
export function mutatedRecovery(state:RecoveryState, ticket:RecoveryTicket, mutation:RecoveryMutation):RecoveryState {
  ...
  let next:RecoveryState = {...state, refreshError:mutation.refresh_error};
  ...
  if (mutation.issue !== null) {
    if (current && mutation.draft !== null) {
      ...
      next = {...next, draft:mutation.draft, draftRevision, touched:false, resetDraft:true, issue:{fault:mutation.issue, ticket:{...ticket, draftRevision}}};
    } else {
